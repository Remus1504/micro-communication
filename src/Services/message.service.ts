...
const addMessage = async (
  ...
  const message: IMessageDocument = (await MessageModel.create(
    data,
  )) as IMessageDocument;
  if (data.hasOffer) {
...
const getUserConversationList = async (
  username: string,
): Promise<IMessageDocument[]> => {
  const query = {
...
const markManyMessagesAsRead = async (
  receiver: string,
  sender: string,
  messageId: string,
): Promise<IMessageDocument> => {
  ...
  const message: IMessageDocument = (await MessageModel.findOne({
    _id: messageId,
