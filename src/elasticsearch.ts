...
const checkConnection = async (): Promise<void> => {
  ...
  while (!isConnected) {
    try {
      ...
      log.info(
        `CommunicationService Elasticsearch health status - ${health.status}`,
      );
      isConnected = true;
    } catch (error) {
      log.error('Connection to Elasticsearch failed. Retrying...');
      log.log('error', 'CommunicationService checkConnection() method:', error);
    }
  }
};
...
async function createIndex(indexName: string): Promise<void> {
  try {
    ...
    log.log('error', 'CommunicationService createIndex() method error:', error);
  }
}
...
const getDocumentCount = async (index: string): Promise<number> => {
  try {
    ...
    log.log(
      'error',
      'CommunicationService elasticsearch getDocumentCount() method error:',
      error,
    );
    return 0;
  }
...
const getIndexedData = async (
  ...
  try {
    ...
    log.log(
      'error',
      'CommunicationService elasticsearch getIndexedData() method error:',
      error,
    );
...
const addDataToIndex = async (
  ...
  try {
    ...
    log.log(
      'error',
      'CommunicationService elasticsearch addDataToIndex() method error:',
      error,
    );
  }
};
...
const updateIndexedData = async (
  ...
  try {
    ...
    log.log(
      'error',
      'CommunicationService elasticsearch updateIndexedData() method error:',
      error,
    );
  }
};
...
const deleteIndexedData = async (
  ...
  try {
    ...
    log.log(
      'error',
      'CommunicationService elasticsearch deleteIndexedData() method error:',
      error,
    );
  }
};
